/*!
 * Vendored from canvas-ui (https://canvasui.dev) — MIT + Commons Clause, (c) 2026 David Haz.
 * Used as part of this website, per the licence. Do not redistribute the components themselves.
 * Regenerate with: node agent/canvas-vendor.mjs
 */
...
export function createHexFloat(
  ...
  function createSimProgram(fragSource        )             {
  ...
  function bindSimTexture(texture              , unit        )         {
  ...
  function createPostProgram(fragSource        )             {
    const prog = gl .createProgram() ;
  ...
  function createPostTarget(width        , height        )         {
    const texture = gl .createTexture() ;
    gl .bindTexture(gl .TEXTURE_2D, texture);
